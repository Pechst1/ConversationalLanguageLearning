...
This is the part that makes a persistent-cast AI comic actually work. Without locked references, the AI redraws the characters slightly differently every panel and the illusion collapses. **DB-1 is delivered for the current cast; DB-2 is delivered for every recurring location except `brocante`.**

### B1. Character model sheets (the core asset — one per cast member)
For each recurring character (`user`, `marin_leveque`, `lila_bonnet`, `augustin_de_roncourt`, `romy_tremblay`, `margaux_barman`, `landlord_marchand`), maintain a **reference/model sheet**:
...
"visual_design": {
  ...
  "locations": {
    "le_mistral": {
      ...
      "reference_images": ["assets/serial/locations/le_mistral-booth.png", "assets/serial/locations/le_mistral-counter.png"]
    }
  }
}
```

...
def test_notebook_entry_remembers_last_mode() -> None:
    ...
    assert "router.replace(mode === 'vocabulary' ? '/vocabulary' : '/grammar')" in entry
    assert '<NotebookModeSwitch active="grammar"' in entry
...
def test_grammar_and_vocabulary_pages_share_notebook_switch() -> None:
    ...
    assert 'href="/vocabulary">Words</Link>' in grammar
...
def test_primary_notebook_nav_uses_smart_entrypoint() -> None:
    ...
    assert "NOTEBOOK_MODE_STORAGE_KEY" in masthead
    assert "const [notebookHref, setNotebookHref] = React.useState('/grammar')" in masthead
    assert "mode === 'vocabulary' ? '/vocabulary' : '/grammar'" in masthead
    assert 'href={notebookHref}>Notebook</Link>' in masthead

...
export interface StoryBase {
  id: string;  // String ID in main repo (e.g., "petit_prince")
  story_key?: string;  // Optional for backwards compatibility
  title: string;
  subtitle?: string | null;  // Main repo uses subtitle
  description?: string | null;
  source_book?: string | null;  // Main repo source tracking
  source_author?: string | null;
  difficulty_level?: string | null;
  target_levels?: string[];  // Main repo uses array of levels
  estimated_duration_minutes: number | null;
  theme_tags?: string[];
  themes?: string[];  // Main repo uses themes
  vocabulary_theme?: string | null;
  cover_image_url: string | null;
  author?: string | null;
  total_chapters?: number;
  is_published?: boolean;
  is_active?: boolean;  // Main repo field
}

export interface ChapterBase {
  id: string;  // String ID in main repo (e.g., "chapter1_arrival")
  story_id: string;
  chapter_key?: string;  // Optional for backwards compatibility
  order_index: number;  // Main repo uses order_index
  sequence_order?: number;  // Optional for backwards compatibility
  title: string;
  target_level?: string | null;  // Main repo field
  synopsis?: string | null;
  opening_narrative?: string | null;
  learning_focus?: Record<string, any> | null;  // Main repo field
  min_turns?: number;
  max_turns?: number;
  narrative_goals?: NarrativeGoal[];
  completion_criteria?: CompletionCriteria | null;
  branching_choices?: BranchingChoice[] | null;
  default_next_chapter_id?: string | null;
...
export interface BranchingChoice {
  ...
  text: string;
  hint?: string;
  next_chapter_id?: string;  // String ID
}
...
export interface ChapterCompletionResponse {
  ...
  next_chapter_id: string | null;  // String ID
...
export function useChapter(storyId: string | null, chapterId: string | null) {

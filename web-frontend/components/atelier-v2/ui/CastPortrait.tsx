/**
 * WP-77 — a character's face, round, in one of four moods (WP-D8 added
 * `moved`). WP-D2 moved it here from `components/onboarding/Portrait.tsx`
 * (which still re-exports it) so the journey, the Courrier and onboarding
 * share one face.
 *
...
export type CastPortraitProps = {
  ...
  alt?: string;
  /**
   * WP-D2: ring the disc in the character's own `--char-*` accent instead of
...
/** WP-D2: the drawn cast's `--char-*` key, so an id alone still finds its accent. */
...
export function CastPortrait({ characterId, name, mood = 'neutral', size = 'md', alt = '', ring = false }: CastPortraitProps) {
  ...
  const px = PX[size];
  const accent =
    characterAccent(name || characterId.replace(/_/g, ' ')) ??
    (castId ? characterAccent(ACCENT_KEY[castId]) : undefined);
  const style = {
    width: px,
    height: px,
